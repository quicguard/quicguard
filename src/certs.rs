...
pub fn generate_certificates(cert_path: &Path, key_path: &Path, hostname: &str) -> Result<()> {
...
pub fn load_certificate(path: &Path) -> Result<Vec<rustls::Certificate>> {
    let cert_pem = fs::read(path).context("Failed to read certificate file")?;
...
pub fn load_private_key(path: &Path) -> Result<rustls::PrivateKey> {
    let key_pem = fs::read(path).context("Failed to read private key file")?;
